...
class Usage(BaseModel):
    ...
    cache_creation_input_tokens: Optional[int] = Field(default=0, description="Cache creation input tokens")
    cache_read_input_tokens: Optional[int] = Field(default=0, description="Cache read input tokens")
    input_tokens: Optional[int] = Field(default=0, description="Input tokens")
    output_tokens: Optional[int] = Field(default=0, description="Output tokens")
...
class ToolUseContent(ContentBlock):
    ...
    input: Union[str, Dict[str, Any]] = Field(default="", description="The input parameters")
...
class ToolResultContent(ContentBlock):
    ...
    content: Union[str, Dict[str, Any], List[Union[str, Dict[str, Any]]]] = Field(
...
class Event(BaseModel):
    ...
    content: Union[TextContent, List[Union[ThinkingContent, ToolUseContent, ToolResultContent, TextContent]]] = Field(
        description="The content of the event"
    )
    
    def tail(self) -> ContentBlock:
    ...
    def transform_api(self) -> Dict[str, Any]:
        ...
        if isinstance(self.content, ContentBlock):
            ...
            for item in self.content:
                ...
                if item_dict.get("type") == "tool_use" and "input" in item_dict:
                    if isinstance(item_dict["input"], str):
                        try:
