...
from anthropic.types import MessageDeltaUsage, TextBlock, TextDelta, ToolUseBlock, ThinkingBlock, ThinkingDelta
...
class AsyncAgentRuntime:
    ...
    def __init__(self, project_root: Path, config: Optional[AgentConfig] = None):
    ...
    def _format_tools(self) -> List[Dict[str, Any]]:
        ...
        tool_definitions = [
            {
                "name": "read_file",
                "description": "Read the contents of a file. Use agent-perspective paths like /workspace/file.txt",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "path": {
                            "type": "string",
                            "description": "The file path from agent's perspective (e.g., /workspace/file.txt)"
                        },
                        "start_line": {
                            "type": "integer",
            ...
            {
                "name": "sync_context",
                "description": "更新 context window 并清空临时对话历史。你需要全量生成新的 context 内容，包括任务状态、工作记忆、观察结果等。",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "new_context_content": {
                            "type": "string",
                            "description": "新的 context window 完整内容，应包含 Current Task、Working Memory、Active Observations、Next Steps 等部分"
                        }
                    },
    ...
    async def _load_system_context(self) -> str:
        ...
        structure = workspace_structure_path.read_text(encoding='utf-8') if workspace_structure_path.exists() else ""
...
"""
    ...
    async def create_event(
        ...
        content: Union[TextContent, List[Union[ThinkingContent, ToolUseContent, ToolResultContent, TextContent]]]
    ) -> Event:
        ...
        async for _chunk in stream:
            ...
            if chunk_type == ChunkType.ContentBlockDelta:
                ...
                if content_block_type == ContentDeltaType.Text:
                    ...
                    if current_event and isinstance(current_event.content, list) and current_event.content:
            ...
            if chunk_type == ChunkType.ContentBlockStop:
                if current_event and current_event.type == EventType.ToolUse:
                    # Get the ToolUseContent from the list
                    if isinstance(current_event.content, list) and current_event.content:
    ...
    async def _handle_tool_calls_stream(self, tool_uses: List[ToolUseContent]) -> AsyncIterator[Event]:
        ...
        for tool_use in tool_uses:
            ...
            if tool_name == "read_file":
                ...
                mapped_params = {
                    "dir_path": tool_params.get("path", "")
                }
            elif tool_name == "execute_command":
                mapped_params = tool_params  # No mapping needed
            elif tool_name == "sync_context":
                mapped_params = {
                    "new_context_content": tool_params.get("new_context_content", "")
                }
            else:
